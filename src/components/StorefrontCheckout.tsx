...
import { submitStoreOrder, createPaymobIntention, isOnlinePaymentEnabled, type OrderType, type ShippingOption, type StorefrontProduct } from "@/lib/storefront";
import { Loader2, Minus, Plus, Tag, Truck, X, Copy, MessageCircle, MapPin, CreditCard, Banknote } from "lucide-react";
...
export default function StorefrontCheckout({ store, options, cart, onClose, onQuantity }: { store: { id: string; name: string; minimumOrder?: number }; options: ShippingOption[]; cart: CartLine[]; onClose: () => void; onQuantity: (id: string, quantity: number) => void }) {
  ...
  useEffect(() => {
    isOnlinePaymentEnabled(store.id).then(setOnlinePaymentEnabled).catch(() => setOnlinePaymentEnabled(false));
  }, [store.id]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      // If online payment was selected, redirect to Paymob
      if (form.type === "online_payment") {
        setProcessingPayment(true);
        try {
          const paymentResult = await createPaymobIntention(
            result.id,
            Math.round(total * 100), // Convert to cents
            form.name,
            form.phone,
            cart.map((line) => ({ name: line.product.title, amount: Math.round(line.product.display_price * 100) }))
          );
          // Redirect to Paymob checkout
          window.location.href = paymentResult.checkout_url;
          return;
        } catch (payErr: any) {
          setError(payErr.message ?? "تعذر بدء عملية الدفع. جرب تاني أو اختار الدفع عند الاستلام.");
  ...
  return <div className="fixed inset-0 z-30 overflow-y-auto bg-[#040806]/95 p-4 text-white backdrop-blur-sm"><div className="mx-auto my-5 max-w-xl rounded-3xl border border-white/10 bg-[#0a1713] p-5 sm:p-7">
    {done ? (() => {
      ...
      <div className="py-12 text-center">
        ...
        <p className="mt-4 text-lg font-bold">جاري التوجيه لصفحة الدفع...</p>
        <p className="mt-2 text-sm text-slate-400">لا تغلق الصفحة</p>
      </div>
      ...
      <div className="mt-5">
        <span className="text-sm text-slate-300">طريقة الدفع</span>
        <div className="mt-2 grid grid-cols-2 gap-2">
          <Button type="button" variant={form.type === "cash_on_delivery" ? "default" : "outline"} onClick={() => setForm({ ...form, type: "cash_on_delivery" })} className="gap-2">
            <Banknote className="h-4 w-4" /> الدفع عند الاستلام
          </Button>
          {onlinePaymentEnabled && <Button type="button" variant={form.type === "online_payment" ? "default" : "outline"} onClick={() => setForm({ ...form, type: "online_payment" })} className="gap-2">
            <CreditCard className="h-4 w-4" /> ادفع أونلاين
          </Button>}
        </div>
      </div>
      ...
      <Button className="mt-5 w-full" disabled={sending || belowMinimum}>{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : form.type === "online_payment" ? "الانتقال للدفع" : "إرسال الطلب"}</Button>
    </form>}
  </div></div>;
}
