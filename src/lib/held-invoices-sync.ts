...
export async function pushHeldInvoice(invoice: HeldInvoice): Promise<void> {
  ...
  if (!user_id) return;
  await table("held_invoices").upsert({
    id: invoice.id,
    user_id,
    ...
    sale_type: invoice.saleType || "cash",
    items: invoice.items,
    total: invoice.total,
    down_payment: invoice.downPayment ? Number(invoice.downPayment) : null,
    monthly_installment: invoice.monthlyInstallment ? Number(invoice.monthlyInstallment) : null,
    installment_count: invoice.installmentCount ? Number(invoice.installmentCount) : null,
    notes: invoice.notes || null,
    discount_pct: invoice.discountPct ? Number(invoice.discountPct) : null,
    discount_amt: invoice.discountAmt ? Number(invoice.discountAmt) : null,
    tax_pct: invoice.taxPct ? Number(invoice.taxPct) : null,
...
export async function removeHeldInvoice(id: string): Promise<void> {
  const user_id = await uid();
  if (!user_id) return;
  await table("held_invoices").delete().eq("user_id", user_id).eq("id", id);
}
